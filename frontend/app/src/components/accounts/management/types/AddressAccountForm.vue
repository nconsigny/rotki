...
const tags = computed<string[]>({ get() { const model = modelValue.value; return (model.mode === 'edit' ? model.data.tags : model.data.length > 0 ? model.data[0].tags : null) ?? []; }, set(tags: string[]) { const model = modelValue.value; const tagData = tags.length > 0 ? tags : null; if (model.mode === 'edit') { modelValue.value = { ...model, data: { ...model.data, tags: tagData } }; } else { modelValue.value = { ...model, data: [...model.data.map(item => ({ ...item, tags: tagData }))] }; } } });
const label = computed<string>({ get() { const model = modelValue.value; return (model.mode === 'edit' ? model.data.label : model.data.length > 0 ? model.data[0].label : null) ?? ''; }, set(label: string) { const model = modelValue.value; if (model.mode === 'edit') { modelValue.value = { ...model, data: { ...model.data, label } }; } else { modelValue.value = { ...model, data: [...model.data.map(item => ({ ...item, label }))] }; } } });
...
const selectedAddresses = computed(() => {
  return Array.from(selectedAddressIndices.value)
    .map(index => {
...
<template>
  <div class="flex flex-col gap-6">
    <RuiSwitch v-if="showHardwareToggle" v-model="useHardware" label="Add from Hardware Wallet" />
    <template v-if="useHardware">
      <RuiRadioGroup v-model="hardwareType">
        <RuiRadio value="ledger" label="Ledger" />
        <RuiRadio value="trezor" label="Trezor" />
      </RuiRadioGroup>
      <RuiButton :loading="deriving" @click="deriveAddresses">Derive Addresses</RuiButton>
      
      <!-- Hardware Wallet Addresses Confirmed -->
      <div v-if="useHardware && !showAddressSelection && addresses.length > 0" class="p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg">
      ...
      <div v-if="showAddressSelection" class="border border-rui-grey-300 dark:border-rui-grey-600 rounded-lg p-6 bg-white dark:bg-rui-grey-900">
        <div class="flex justify-between items-center mb-6">
          <div>
            <h3 class="text-lg font-semibold text-rui-text dark:text-rui-text-dark">
              Select Addresses 
              <span v-if="!deriving" class="text-rui-primary">({{ selectedAddresses.length }} selected)</span>
            </h3>
            <p v-if="selectedAddresses.length > 0" class="text-sm text-rui-text-secondary dark:text-rui-text-secondary-dark mt-1">
        ...
        <!-- Loading State -->
        <div v-if="deriving && derivedHardwareAddresses.length === 0" class="flex flex-col items-center justify-center py-12">
          <RuiProgress circular indeterminate color="primary" class="mb-4" />
          ...
          <table class="w-full border-collapse border border-rui-grey-300 dark:border-rui-grey-600 rounded-lg overflow-hidden">
            <thead>
              <tr class="bg-rui-grey-50 dark:bg-rui-grey-800">
                <th class="border-b border-rui-grey-300 dark:border-rui-grey-600 px-4 py-3 text-left text-rui-text dark:text-rui-text-dark font-medium">Select</th>
                <th class="border-b border-rui-grey-300 dark:border-rui-grey-600 px-4 py-3 text-left text-rui-text dark:text-rui-text-dark font-medium">Index</th>
                <th class="border-b border-rui-grey-300 dark:border-rui-grey-600 px-4 py-3 text-left text-rui-text dark:text-rui-text-dark font-medium">Address</th>
                <th class="border-b border-rui-grey-300 dark:border-rui-grey-600 px-4 py-3 text-left text-rui-text dark:text-rui-text-dark font-medium">Derivation Path</th>
              </tr>
            </thead>
            <tbody>
              <!-- Loading rows while deriving more addresses -->
              <template v-if="deriving && derivedHardwareAddresses.length > 0">
                <tr v-for="n in 3" :key="`loading-${n}`" class="animate-pulse">
                  <td class="border-b border-rui-grey-200 dark:border-rui-grey-700 px-4 py-3">
                    <div class="w-4 h-4 bg-rui-grey-300 dark:bg-rui-grey-600 rounded"></div>
                  </td>
                  <td class="border-b border-rui-grey-200 dark:border-rui-grey-700 px-4 py-3">
                    <div class="w-8 h-4 bg-rui-grey-300 dark:bg-rui-grey-600 rounded"></div>
                  </td>
                  <td class="border-b border-rui-grey-200 dark:border-rui-grey-700 px-4 py-3">
                    <div class="w-80 h-4 bg-rui-grey-300 dark:bg-rui-grey-600 rounded"></div>
                  </td>
                  <td class="border-b border-rui-grey-200 dark:border-rui-grey-700 px-4 py-3">
                    <div class="w-32 h-4 bg-rui-grey-300 dark:bg-rui-grey-600 rounded"></div>
                  </td>
                </tr>
              </template>
              ...
              <tr v-if="!deriving && derivedHardwareAddresses.length === 0">
                <td colspan="4" class="px-4 py-8 text-center text-rui-text-secondary dark:text-rui-text-secondary-dark">
        ...
        <!-- Pagination -->
        <div v-if="totalPages > 1 && !deriving" class="flex justify-center items-center gap-2 mt-6">
          <RuiButton 
            size="sm" 
            variant="outlined" 
        ...
        <div v-if="selectedAddresses.length > 0 && !deriving" class="mt-6 p-4 bg-rui-grey-50 dark:bg-rui-grey-800 rounded-lg">
    ...
    <ModuleActivator v-if="[Blockchain.ETH, 'evm'].includes(modelValue.chain) && !editMode" @update:selection="selectedModules = $event" />
    <div class="flex flex-col gap-4">
      <AddressInput ref="address" v-model:addresses="addresses" v-model:error-messages="errors" :disabled="loading || editMode" :multi="!editMode" :show-wallet-import="showWalletImport.value" />
      <AccountDataInput v-model:tags="tags" v-model:label="label" :disabled="loading" />
    </div>
  </div>
</template>
